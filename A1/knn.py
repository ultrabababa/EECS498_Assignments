...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    Xtr_rows = x_train.reshape(num_train, x_shape[1]*x_shape[2]*x_shape[3])
    Xte_rows = x_test.reshape(num_test, x_shape[1]*x_shape[2]*x_shape[3])
    for i in range(num_train):
        for j in range(num_test):
            difference_row = Xtr_rows[i] - Xte_rows[j]
            # print("difference_row:", difference_row)
            squared_difference_row = difference_row.pow(2)
            # print("squared_difference_row:", squared_difference_row)
            sum_squared_value = squared_difference_row.sum()
            # print("sum_squared_value:", sum_squared_value)
            dists[i, j] = sum_squared_value.sqrt().item()
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        difference_row = Xtr_rows[i] - Xte_rows
        # print(difference_row)
        squared_difference_row = difference_row.pow(2)
        # print("squared_difference_row:", squared_difference_row)
        sum_squared_value = squared_difference_row.sum(dim=1)
        # print("sum_squared_value:", sum_squared_value)
        dists[i] = sum_squared_value.sqrt()
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    Xtr_rows = x_train.view(num_train, -1)
    Xte_rows = x_test.view(num_test, -1)
    train_squared = torch.sum(Xtr_rows ** 2, dim=1).unsqueeze(1)  # Shape: (num_train, 1)
    test_squared = torch.sum(Xte_rows ** 2, dim=1).unsqueeze(0)    # Shape: (1, num_test)
    # (a - b)**2 = a**2 + b**2 - 2AB
    cross_term = torch.mm(Xtr_rows, Xte_rows.t())
    dists = (train_squared + test_squared - 2 * cross_term).sqrt()  # Broadcasting happens here
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
        _, topk_indices = torch.topk(dists[:, i], k=k, largest=False)
        topk_labels = y_train[topk_indices]
        labels_count = torch.bincount(topk_labels, minlength=y_train.max() + 1)
        most_label = torch.argmax(labels_count) # return the former index if there is a tie
        y_pred[i] = most_label
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
class KnnClassifier:
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        y_test_pred = predict_labels(compute_distances_no_loops(self.x_train, x_test=x_test), self.y_train, k=k)
        ######################################################################
        #                         END OF YOUR CODE                           #
        ######################################################################
        return y_test_pred
...
def knn_cross_validate(
    ...
    # k_to_accuracies[k] should be a list of length num_folds giving the  [1,2,3,4,5]
    ...
    for k in k_choices:
        ...
        for fold_index in range(num_folds):
            ...
            x_validation = x_train_folds_local[fold_index]
            y_validation = y_train_folds_local[fold_index]
            del x_train_folds_local[fold_index]
            del y_train_folds_local[fold_index]
            x_train_other_folds = torch.cat(x_train_folds_local, dim=0)
            y_train_other_folds = torch.cat(y_train_folds_local, dim=0)

            knn = KnnClassifier(x_train_other_folds, y_train_other_folds)
            accuracy_list.append(knn.check_accuracy(x_validation, y_validation, k=k))
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    # Replace "pass" statement with your code
    new_dict = {}
    for k, accuracy in sorted(k_to_accuracies.items()):
        new_dict[k] = sum(accuracy) / len(accuracy)
