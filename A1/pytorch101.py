...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    for idx0 in torch.arange(B):
        z[idx0] = torch.mm(x[idx0], y[idx0])
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
    return z
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    # step 1: compute mean
    M, N = x.shape
    columns_sum = torch.sum(x, dim=0) # reduction operation shape is (1, N)
    mean = columns_sum / M # shape is (1, N)

    # step 2: compute standard deviation
    diff = x - mean # broadcast (1, N) -> (M, N)
    diff_square = diff ** 2
    variance = torch.sum(diff_square, dim=0) / (M - 1) # reduction operation shape is (1, N)
    sigma = torch.sqrt(variance)

    # step 3: compute tensor
    y = (x - mean) / sigma
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
